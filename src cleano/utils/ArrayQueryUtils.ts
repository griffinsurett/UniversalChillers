...
function tryInsertItem(tree: QueryItemTree[], item: QueryItemTree): boolean {
  if (item.parent) {
    let parentItem =
      findQueryItem(tree, item.parent) ||
      findQueryItemBySlug(tree, item.parent);
    if (parentItem) {
...
function buildDynamicTree(flatItems: QueryItem[]): QueryItemTree[] {
  const items: QueryItemTree[] = flatItems.map((rawItem) => {
    ...
    const finalLabel =
      item.label ||
      (item.slug ? capitalize(normalizeSlug(item.slug)) : capitalize(item.id));
  ...
  items.forEach((item) => {
    if (item.parent) {
      const parentItem =
        lookup[item.parent] || findQueryItemBySlug(tree, item.parent);
      if (parentItem) {
        parentItem.children.push(item);
      } else {
        tree.push(item);
 ...
 */
export async function buildContentQueries(baseQueries: {
  [key: string]: QueryItemTree[];
 ...
 */
export async function gatherDynamicQueries(): Promise<{
